...
export const accountTypeSchema = z.object({
  ...
  billingType: z.enum(["Consumidor Final", "Responsable Inscripto", "Monotributista"]).optional(),
  billingAddress: z
    .object({
      country: z.string(),
      town: z.string(),
...
export function useOnboarding() {
  const [currentStep, setCurrentStep] = useState<OnboardingStep>(() => {
    try {
      ...
      console.log("[v0] Hook initialization - savedStep from localStorage:", savedStep)
      const initialStep = (savedStep as OnboardingStep) || "registration"
      console.log("[v0] Hook initialization - using step:", initialStep)
      return initialStep
    } catch {
      console.log("[v0] Hook initialization - error reading localStorage, using registration")
      return "registration"
    }
  })
  ...
  // El estado ya se inicializa correctamente desde localStorage

  const persistData = useCallback((newData: OnboardingData, step?: OnboardingStep) => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(newData))
      if (step) {
        console.log("[v0] Persisting step to localStorage:", step)
        console.log("[v0] Stack trace for persistence:", new Error().stack?.split("\n").slice(1, 4).join("\n"))
        localStorage.setItem(STEP_KEY, step)
        console.log("[v0] Verification - localStorage now contains:", localStorage.getItem(STEP_KEY))
      }
    } catch (error) {
  ...
  const nextStep = useCallback(() => {
    ...
    if (currentIndex < steps.length - 1) {
      ...
      // Verificar inmediatamente que se persistió correctamente
      setTimeout(() => {
        const verifyStep = localStorage.getItem(STEP_KEY)
        console.log("[v0] Verification after nextStep - localStorage contains:", verifyStep)
  ...
  const login = useCallback(async () => {
    try {
      const baseUrl = process.env.NEXT_PUBLIC_MUBEE_API_URL || "https://api.mubee-platform.com"
  ...
  const apiCall = useCallback(
    async (endpoint: string, method: "GET" | "POST" | "PUT" = "POST", body?: any) => {
      setIsLoading(true)
      setError(null)
