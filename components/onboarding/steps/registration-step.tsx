...
export function RegistrationStep() {
  const { updateStepData, nextStep, apiCall, isLoading, error, setError } = useOnboarding()
  ...
  const checkEmailVerification = async (email: string) => {
    try {
      ...
      if (response.userStatus === "AWAITING_REGISTRATION_COMPLETE") {
        ...
        updateStepData("step1", { email, verified: true })
        ...
        setTimeout(() => {
          console.log("[v0] Calling nextStep()...")
          nextStep()
        }, 1000)
  ...
  const handleManualVerification = async () => {
    if (userEmail) {
      ...
      try {
        await checkEmailVerification(userEmail)
      } finally {
    ...
    return (
      <div className="space-y-6 text-center">
        ...
        <div className="space-y-3">
          <Button onClick={openEmailApp} className="w-full bg-transparent" variant="outline">
            <Mail className="h-4 w-4 mr-2" />
            Abrir email
          </Button>

          <Button onClick={handleManualVerification} className="w-full" disabled={isManualVerifying}>
          ...
          {(isCheckingEmail || isManualVerifying) && (
            <div className="flex items-center justify-center space-x-2 text-sm text-muted-foreground">
              ...
              <span>{isManualVerifying ? "Verificando estado..." : "Verificando automáticamente..."}</span>
            </div>
          )}
        </div>
        ...
        <div className="text-xs text-muted-foreground space-y-2">
          ...
          <p>La verificación se detectará automáticamente cuando hagas clic en el enlace</p>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input id="referralCode" placeholder="Código de referido" {...register("referralCode")} />
        </div>
